...
//            }
            ...
            NSString *userID = conversation.latestMessage.from;
            model.title = [HSUserInfoCacheTool nickNameForUser:userID];
            model.avatarURLPath = [HSUserInfoCacheTool avatarUrlForUser:userID];
        }
...
//            latestMessageTitle = [NSString stringWithFormat:@"%@: %@", from, latestMessageTitle]; // yc环信 注释掉，不然显示 xxx：你吃饭了嘛
        }
